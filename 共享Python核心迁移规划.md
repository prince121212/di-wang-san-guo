...
1. `local-lane`：设置、日志、缓存状态和任务状态，只做本地工作，永远不等待网络、重试退避或定时睡眠。读取优先使用内存快照，写入串行并采用原子替换。
...
- 建立独立 `local-lane`、`event-lane` 和 `network-lane(accountId)`。
