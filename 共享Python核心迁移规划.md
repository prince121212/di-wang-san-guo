...
> 状态：已完成基础设施和首条路由切换（`GET /api/health`）。其他路由仍按所有者矩阵保持旧实现，待阶段 6–8 纵向迁移，不代表 Android 旧 Kotlin 网络路径已全部消失。
...
> 状态：已完成。共享生命周期、状态机、非敏感账号账本、Keystore 密码/Session 端口和双端主时间线均已接入；详细证据见 `docs/core_migration/account_state_stage6.md`。
...
> 状态：执行中。Bridge 分通道和 Android 账号列表共享投影已完成；军情已建立首个持久化网络 operation 样板，真实成功路径待用户授权验证；详细记录见 `docs/core_migration/android_host_stage7.md`。
...
> 状态：已完成基础拆分与 JVM 隔离验证。

- 审计 `AssistantWebBridge` 的单 executor、Controller 和 Service 调用链。
- 建立独立 `local-read`、`local-write`、`event-lane` 和 `network-lane(accountId)`。
- 先用模拟长任务证明本地调用不会排队，再切换业务路由。

### 工作项 T7.2：迁移本地快速路由

> 状态：执行中；`GET /api/accounts`、`GET /api/accounts/settings` 已切换，`POST /api/military/future/save`、`POST /api/liubu/save` 和 `POST /api/formations/save` 已成为两端共享的本地设置写入切片，其余设置、日志、任务状态和缓存路由待继续。

- 优先迁移账号展示、设置读取/保存、日志、任务状态和缓存快照。
- 本地保存必须真实落盘后才返回“已保存”，且不触发或等待游戏网络。
- 在真实 WebView Bridge 上验收 5.5 的点击反馈和本地读写指标。

### 工作项 T7.3：建立军情网络异步样板

> 状态：离线 operation、前端轮询和失败关闭已完成；未经用户授权不进行真实成功路径验证。

- 页面点击后立即显示“正在刷新军情”。
- Bridge 快速返回持久化 `operationId`，按账号网络通道等待游戏服务器。
- 完成结果由事件或 `operation/status` 更新，页面重建后仍可恢复。
- 未经用户明确授权，只做离线 fixture 与模拟网络验证，不访问游戏服务器。
