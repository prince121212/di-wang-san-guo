...
- 可靠后台：按最近任务期限唤醒；空闲期间不持有 WakeLock，仅在单次执行窗口内短时持有，并由 Handler、系统闹钟和执行窗口恢复看门狗共同兜底。
...
- 调度按任务期限自适应；空闲期间释放 WakeLock，单次执行窗口使用有超时的短锁；低延迟 Handler、`setExactAndAllowWhileIdle`/`setAndAllowWhileIdle` 截止时间闹钟，以及执行窗口恢复看门狗互为备份。
- 攻略页实时检测通知、电池优化豁免、系统后台限制和精确闹钟；常见厂商额外提供后台策略与自启动设置入口，并明确提示这些开关无法由 Android 公共 API 查询或自动授予。
