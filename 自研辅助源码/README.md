...
ANDROID_ID 的哈希（不需要权限，卸载重装不变，恢复出厂才会变），每台手机只有第一个注册的账号
送；旧版 App 不上传 ANDROID_ID，按本次安装的设备密钥判断。不按 IP 限制（2026-09-27 起，
共享 IP 会误伤新用户）。服务端只保存密钥哈希，不存原始 ANDROID_ID。未送的原因写在管理后台该
会员的操作记录里（例如“同一台手机已由 xx 注册”），管理员可用“体验 · 1天”补发。
