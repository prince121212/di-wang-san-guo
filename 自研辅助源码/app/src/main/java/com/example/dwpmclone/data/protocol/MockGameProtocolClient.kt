...
/** Mock-only protocol client. Replace only after authorized runtime DEX/protocol recovery. */
class MockGameProtocolClient : GameProtocolClient {
    ...
    override suspend fun withdrawMineDefense(session: GameSession, mineId: Long) = ProtocolResult.Ok(StepResult(true, "mock withdraw"))
    ...
    override suspend fun scanAlarmAndMaybeWithdraw(session: GameSession, config: AlarmWithdrawConfig) = ProtocolResult.Ok(StepResult(true, "mock alarm scan; withdraw=${config.withdrawDefense} protected=${config.mockOnlyProtection}"))
