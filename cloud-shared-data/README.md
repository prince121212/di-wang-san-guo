...
新用户注册成功即送 1 天体验（`plan: "trial"`），每台手机只送首个注册的账号。按两把钥匙判断：
APP 上传的 ANDROID_ID 哈希，以及设备签名密钥（兼顾不上传 ANDROID_ID 的旧版 APP）；任一已被其他
账号先注册过就不送。不按 IP 判断（2026-09-27 起）：运营商共享出口和家庭/宿舍 Wi-Fi 会误伤新用户。
每把钥匙是一个 `member-trial-v1:<HMAC>` Durable Object，只存首个注册者，永不过期、不存原始设备标识。未送原因写入该会员审计（管理员可见，APP 不可见），管理员可用
